...
async function main() {
  ...
  const captures = [
    {
      ...
      run: async (page) => {
        ...
        await page.waitForSelector(`a[href*="${YEARBOOK_A}"]`);
        await page.click(`a[href*="${YEARBOOK_A}"]`);
        await page.waitForSelector("text/Legacy & Horizons", { timeout: 10000 });
    ...
    },
    {
      ...
      run: async (page) => {
        ...
        await page.waitForSelector("text/You don't have access to this yearbook", {
          timeout: 10000,
        });
    ...
    },
    {
      ...
      run: async (page) => {
        ...
        await page.waitForSelector("text/Filter by status", { timeout: 10000 });
        await new Promise((r) => setTimeout(r, 1200));
      },
    },
    {
      ...
      run: async (page) => {
        ...
        await page.waitForSelector("text/Proofing Center", { timeout: 10000 });
        await new Promise((r) => setTimeout(r, 1200));
      },
    },
  ];
  ...
  for (const c of captures) {
    console.log(`\n---> Capturing [${c.desc}] (${c.name})...`);
    const page = await browser.newPage();
    try {
      const token = await getSessionCookie(c.email, accounts[c.email].password);
      ...
      const dest = path.join(SCREENSHOT_DIR, c.name);
      fs.writeFileSync(dest, buf);
      console.log(`  ✔ Saved ${c.name} (${buf.length} bytes)`);
    } catch (e) {
      console.error(`  ✘ Error capturing ${c.name}:`, e.message);
    } finally {
      await page.close();
    }
  }
