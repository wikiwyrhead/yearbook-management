...
async function main() {
  if (!fs.existsSync(PDF_PATH)) {
    ...
    console.error("Please set ICAS_BLUEPRINT_PDF_PATH in .env to point to your local private PDF file.");
    process.exit(1);
  }
  console.log("================================================================================");
  ...
  try {
    ...
    const catAcademic = (await client.query(`
      ...
      RETURNING id;
    `, [yearbookId])).rows[0]?.id || (await client.query(`SELECT id FROM public.section_categories WHERE yearbook_id = $1 AND code = 'ACADEMIC_PROFILES'`, [yearbookId])).rows[0].id;

    const catInst = (await client.query(`
      INSERT INTO public.section_categories (yearbook_id, name, code, color, sort_order)
      VALUES ($1, 'Institutional & Governance', 'INSTITUTIONAL', '#8b5cf6', 2)
      ON CONFLICT (id, yearbook_id) DO NOTHING
      RETURNING id;
    `, [yearbookId])).rows[0]?.id || (await client.query(`SELECT id FROM public.section_categories WHERE yearbook_id = $1 AND code = 'INSTITUTIONAL'`, [yearbookId])).rows[0].id;

    const catLife = (await client.query(`
      INSERT INTO public.section_categories (yearbook_id, name, code, color, sort_order)
      VALUES ($1, 'Student Life & Activities', 'STUDENT_LIFE', '#10b981', 3)
      ON CONFLICT (id, yearbook_id) DO NOTHING
      RETURNING id;
    `, [yearbookId])).rows[0]?.id || (await client.query(`SELECT id FROM public.section_categories WHERE yearbook_id = $1 AND code = 'STUDENT_LIFE'`, [yearbookId])).rows[0].id;

    const catCommunity = (await client.query(`
      INSERT INTO public.section_categories (yearbook_id, name, code, color, sort_order)
      VALUES ($1, 'Community & Tributes', 'COMMUNITY', '#f59e0b', 4)
      ON CONFLICT (id, yearbook_id) DO NOTHING
      RETURNING id;
    `, [yearbookId])).rows[0]?.id || (await client.query(`SELECT id FROM public.section_categories WHERE yearbook_id = $1 AND code = 'COMMUNITY'`, [yearbookId])).rows[0].id;

    const ltPortrait = (await client.query(`
      ...
      ON CONFLICT (id, yearbook_id) DO NOTHING
      RETURNING id;
    `, [yearbookId])).rows[0]?.id || (await client.query(`SELECT id FROM public.layout_types WHERE yearbook_id = $1 AND code = 'PORTRAIT_GRID'`, [yearbookId])).rows[0].id;

    const ltFeature = (await client.query(`
      INSERT INTO public.layout_types (yearbook_id, name, code, default_span, sort_order)
      VALUES ($1, 'Feature Spread', 'FEATURE_SPREAD', 'two_page_spread', 2)
      ON CONFLICT (id, yearbook_id) DO NOTHING
      RETURNING id;
    `, [yearbookId])).rows[0]?.id || (await client.query(`SELECT id FROM public.layout_types WHERE yearbook_id = $1 AND code = 'FEATURE_SPREAD'`, [yearbookId])).rows[0].id;
    ...
    const sectionsDef = [
      ...
      { name: "Senior High School Graduation & Awardees", start: 16, end: 19, pos: 5, cat: catAcademic },
      ...
      { name: "Junior High School Moving Up & Awardees", start: 48, end: 51, pos: 10, cat: catAcademic },
      { name: "Grade 10 Profiles — St. Peter, St. Paul, St. Pio", start: 52, end: 75, pos: 11, cat: catAcademic },
      { name: "Elementary Moving Up & Awardees", start: 76, end: 79, pos: 12, cat: catAcademic },
      { name: "Grade 6 Profiles — St. Rose & St. Regina", start: 80, end: 91, pos: 13, cat: catAcademic },
      { name: "Kindergarten Moving Up & Awardees", start: 92, end: 94, pos: 14, cat: catAcademic },
      { name: "Kinder 2 Profiles — St. Raphael AM & PM", start: 95, end: 105, pos: 15, cat: catAcademic },
      { name: "Faculty, Staff & Undergraduates", start: 106, end: 115, pos: 16, cat: catInst },
      { name: "Student Life, Athletics & School Events", start: 116, end: 127, pos: 17, cat: catLife },
      { name: "Advertisements, Sponsors & Tributes", start: 128, end: 134, pos: 18, cat: catCommunity },
      { name: "Acknowledgements, Press & Editorial Board", start: 135, end: 136, pos: 19, cat: catInst },
    ...
    for (let pageNum = 1; pageNum <= 138; pageNum++) {
      ...
      // Determine Section
      const secDef = sectionsDef.find((s) => pageNum >= s.start && pageNum <= s.end) || sectionsDef[0];
      ...
      const pageRes = await client.query(`
        ...
        RETURNING id;
      `, [yearbookId, sectionId, secDef.cat, ltPortrait, pageNum, displayLabel, isUnnumbered, pageTitle]);
      ...
      if ((pageNum >= 20 && pageNum <= 47) || (pageNum >= 52 && pageNum <= 75) || (pageNum >= 80 && pageNum <= 91) || (pageNum >= 95 && pageNum <= 105)) {
        // Extract words matching name patterns
        const nameCandidates = [];
        for (let idx = 0; idx < rawStrings.length; idx++) {
          const str = rawStrings[idx].trim();
          if (str.length >= 3 && /^[A-ZÑ\s,\.\-]+$/.test(str) && !["MILESTONE", "2024-2025", "2025", "GRADE", "ST.", "STEM", "HUMSS", "ABM", "SESSION"].includes(str)) {
