...
- A genuine Google *API key* cannot list, browse, download, or import private Drive files. It only works for public/unrestricted resources. So the QA report wording ("Organization Google Drive requires GOOGLE_DRIVE_API_KEY", implying an API key) is **incorrect**.
