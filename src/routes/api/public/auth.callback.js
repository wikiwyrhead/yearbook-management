            ...
            GET: async ({ request }) => {
                ...
                try {
                    ...
                    if (state.provider === "box") {
                        ...
                        if (!res.ok)
                            throw new Error(`Box token exchange failed: ${await res.text()}`);
                        tokens = await res.json();
                    }
                    else if (state.provider === "canva") {
                        const authHeader = Buffer.from(`${process.env["CANVA_CLIENT_ID"]}:${process.env["CANVA_CLIENT_SECRET"]}`).toString("base64");
