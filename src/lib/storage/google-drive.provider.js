...
const FILE_FIELDS = "id,name,mimeType,size,modifiedTime,webViewLink,thumbnailLink,parents";
...
function connectionKey(ref) {
    if (ref.connectionKey)
        return ref.connectionKey;
    if (ref.scope === "organization") {
        const key = process.env["GOOGLE_DRIVE_API_KEY"];
        if (key)
            return key;
        throw new ProviderNotConfiguredError("google_drive", "Managed Google Drive connector is not enabled for this project.");
    }
    throw new ProviderNotConfiguredError("google_drive", "This member has not connected their Google Drive via the managed connector.");
}
async function driveFetch(ref, path, params) {
...
async function query(ref, q, opts) {
...
export const googleDriveProvider = {
    ...
    async listFolders(ref, parentId, opts) {
        ...
        const result = await query(ref, `'${parent}' in parents and mimeType = '${FOLDER_MIME}' and trashed = false`, opts);
        return {
    ...
    async listFiles(ref, folderId, opts) {
        ...
        return query(ref, `'${parent}' in parents and mimeType != '${FOLDER_MIME}' and trashed = false`, opts);
    },
    async searchFiles(ref, q, opts) {
        return query(ref, `name contains '${escapeQ(q)}' and mimeType != '${FOLDER_MIME}' and trashed = false`, opts);
    },
