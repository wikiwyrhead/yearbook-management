...
export const getYearbookStorage = createServerFn({ method: "GET" })
    ...
    await requireYearbookMember(context.supabase, context.userId, data.yearbookId);
...
export const saveYearbookStorage = createServerFn({ method: "POST" })
    ...
    await requireYearbookCoordinator(context.supabase, context.userId, data.yearbookId);
...
export const startOAuthFlow = createServerFn({ method: "POST" })
    ...
    if (data.provider === "box") {
        ...
        if (!clientId)
            throw new Error("Box client ID not configured");
        return {
...
export const importProviderFiles = createServerFn({ method: "POST" })
    ...
    await assertImportAllowed(context.supabase, context.userId, data.yearbookId, data.studentId);
