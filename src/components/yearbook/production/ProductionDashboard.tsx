...
export function ProductionDashboard({ yearbookId, canManage }: ProductionDashboardProps) {
  ...
  if (isLoading) return <div className="p-12 text-center animate-pulse">Loading production data...</div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="plate p-4 border-l-4 border-blue-500">
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            {readiness?.ready ? <CheckCircle className="size-5 text-green-500" /> : <XCircle className="size-5 text-destructive" />}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Tabs defaultValue="snapshots">
            <TabsList className="w-full justify-start border-b rounded-none h-auto p-0 bg-transparent">
              <TabsTrigger value="snapshots" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent py-2 px-4">Snapshots</TabsTrigger>
              <TabsTrigger value="submissions" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent py-2 px-4">Submissions</TabsTrigger>
              <TabsTrigger value="history" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent py-2 px-4">Audit Log</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="snapshots" className="mt-6 space-y-4">
              <div className="flex justify-between items-center mb-4">
                ...
                {canManage && (
                  <Button size="sm" onClick={() => mSnapshot.mutate()} disabled={!readiness?.ready || mSnapshot.isPending}>
                    New Snapshot
                  </Button>
                )}
              </div>
              ...
              {data?.snapshots.length === 0 ? (
                ...
                data?.snapshots.map((s: any) => (
                  <div key={s.id} className="plate p-4 flex items-center justify-between group hover:border-primary/50 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => mPackage.mutate(s.id)} disabled={mPackage.isPending}>
                        Bundle Package
                      </Button>
            ...
            <TabsContent value="submissions" className="mt-6">
              ...
              {data?.submissions.length === 0 ? (
                ...
                data?.submissions.map((sub: any) => (
                  <div key={sub.id} className="plate p-4 flex items-center justify-between border-l-4 border-green-500">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-medium">{sub.service_bureaus?.name || "Unknown Bureau"}</span>
                        <Badge variant="secondary" className="text-[10px]">{sub.status}</Badge>
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">
                        Package: {sub.package_id.slice(0, 8)} · Ref: {sub.external_reference || "N/A"}
                      </div>
                    </div>
            ...
            <TabsContent value="history" className="mt-6">
              <div className="plate divide-y">
                ...
                <div className="p-3 text-sm text-muted-foreground text-center">Audit log integrated with production activity.</div>
              </div>
            </TabsContent>
          </Tabs>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="plate overflow-hidden">
            <div className="p-3 bg-muted/50 border-b flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => mPreflight.mutate()}>Refresh</Button>
            </div>
            ...
            <div className="p-4 space-y-3">
              {data?.serviceBureaus.map((sb: any) => (
                <div key={sb.id} className="text-sm border-b pb-2 last:border-0">
                  ...
                  <div className="text-xs text-muted-foreground">Method: {sb.submission_method}</div>
                </div>
              ))}
              {canManage && (
          ...
          {latestPackage && (
            <div className="plate p-4 bg-primary/5 border-primary/20">
              <h4 className="text-xs font-semibold uppercase tracking-wider text-primary mb-2">Latest Package Identity</h4>
              <div className="flex items-center gap-2 mb-3">
                <Download className="size-4 text-primary" />
                <span className="text-sm font-medium truncate">Package v{latestSnapshot?.version}</span>
              </div>
