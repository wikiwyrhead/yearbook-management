    ...
    for (let i = 0; i < files.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(async (file) => {
          try {
            // Real binary upload to local storage backend
            const formData = new FormData();
            formData.append("file", file);
            formData.append("bucket", "yearbook_assets");
            formData.append("yearbookId", yearbookId);
            if (studentId) formData.append("subfolder", `students/${studentId}`);
            ...
            if (!uploadRes.ok) {
              ...
              throw new Error(errData.error || `HTTP ${uploadRes.status} upload failed`);
            }
